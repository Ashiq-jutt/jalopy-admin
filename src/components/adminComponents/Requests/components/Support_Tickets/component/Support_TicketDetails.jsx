...
import { Smiley, Forward } from "./assets";
...
export default function SupportTicketDetail({ accountactive, setRefresh, token, data, setDetailView }) {
  ...
  return (
    <div className={`p-2  flex z-99999 flex-wrap flex-row justify-between overflow-y-auto font-poppins fixed bg-white    ${accountactive ? "  ml-[-50px] w-full md:w-[calc(100%-320px)]" : "md:w-[calc(100%-320px)] w-full"} h-[100vh] md:h-[calc(100%-70px)] top-[70px]  text-main-color`}>
      ...
      <Dialog visible={closeWitoutReply}
        ...
        <div className="flex flex-wrap flex-row mt-4 justify-left gap-2 ">
          ...
          <Button label="No" onClick={() => {
            setCloseWithoutReply(false)
          }} className="bg-main-color text-white p-1 pl-2 pr-3" />
        </div>
      </Dialog>
