...
export default function ListDrivers({sortBy,driversData}){  
  ...
  const tableActions = (rowData) => {
    return (
      <div className="w-full   flex flex-wrap  items-center flex-row justify-left gap-2"> 
          {" "}
          <div
          onClick={()=>{
            setRowData(rowData)  
    ...
    currentPageReportTemplate="Showing {first} to {last} of {totalRecords} entries"
        ...
        e.target.src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTw_HeSzHfBorKS4muw4IIeVvvRgnhyO8Gn8w&s"
           ...
           <Column header="Car Company"   headerClassName="text-white font-normal bg-main-color" className="text-main-color font-normal" body={(rowData)=>{ 
             return   ( 
               <p>{rowData?.vehicle?.company}</p>
             )
           }}></Column>
