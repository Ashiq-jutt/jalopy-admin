        ...
        :undefined
          ...
          <div className="w-full flex flex-wrap flex-row  pl-[5%] pr-[5%] md:pl-[0%] md:pr-[0%] items-center ">
              <div className=" w-[90%] md:w-[170px] mt-4 flex flex-wrap flex-row items-center  justify-left">
                <p>Image {carDetailView?.imagePath ?  <span onClick={()=>{ 
      
      fetch(carDetailView?.imagePath)
      .then(response => {
        const filename = response.headers.get('Content-Disposition')
          ?.split('filename=')[1]
          ?.split(';')[0]
          ?.replace(/"/g, '') || carDetailView?.imagePath?.split('/').pop();
      ...
      .catch(error => console.error('Download failed:', error));
        ...
        <div onClick={()=>{ 
              imgRef.current.click()
                      ...
                      <p className="text-main-color w-[calc(100%-120px)] "> {formik.values.Image ? formik.values.Image.name :"No File Choosen" }</p>
                </div> 
              <input
                ref={imgRef}
