     ...
     useEffect(() => {  
      ...
      data.map(item=>{   
        if(item.label === "Total Drivers"){  
          ...
          let obj2={...item} 
          obj2.no=` ${res?.newDrivers}`   
           statobj.push(obj2)
     } 
     
  else if(item.label === "Active Now"){  
    let obj2={...item} 
    obj2.no=` ${(res?.activeDrivers)}`  
     statobj.push(obj2)
} 
setData(statobj)
      }) 
    ...
    return(  
      ...
      <div className="flex rounded-md shadow-custom p-3  w-full max-w-[985px] pt-5 pb-5 pl-10 pr-10  mt-4 rounded flex-wrap flex-row justify-around md:justify-between">  
         {  
            data.map((stat,index)=>{ 
                const Svg=stat.Svg; 
