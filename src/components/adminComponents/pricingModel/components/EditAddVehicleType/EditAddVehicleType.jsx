...
export default function EditVehicleType({setEditNewDialog,vehicledetails,setRefresh}){      
 ...
 noOfSeats: Yup.string().required("No Of Seats Is Required")   
  ...
  noOfSeats: ""
        ...
        onSubmit: (values, actions) => {    
            setLoader(true) 
            Axios.put(
                `${process.env.REACT_APP_BASE_URL}/api/v1/VehicleType/Update`,formik.values,
                { headers: { Authorization: `Bearer ${token}` } }
              ).then(res=>{  
                setLoader(false)   
                setTimeout(()=>{ 
                    setRefresh(prev=>!prev)
                setEditNewDialog(prev=>!prev)  
      ...
      useEffect(()=>{ 
    formik.setValues(vehicledetails)
      },[])
    return( 
          ...
          />  
            ...
            <p className="text-red-500 w-full text-[14px] mt-1">
              {formik.errors.noOfSeats}
                 ...
                 <div className="w-full flex flex-wrap flex-row justify-center items-center mt-4">
        <Button loading={loader} disabled={loader} label="Update Vehicle Pricing"  className=" bg-main-color text-white font-poppins p-1 pl-2 pr-2"/> 
         </div>   
