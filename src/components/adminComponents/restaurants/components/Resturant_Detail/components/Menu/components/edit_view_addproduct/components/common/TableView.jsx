...
export default function ViewTable({ VendorShopId, itemId, refreshProduct }) {
  ...
  }, [itemId, refresh, refreshProduct])

  return (
    <div>
      {showAiDetailView ? <EditViewAddDrinks VendorShopId={VendorShopId} setRefresh={setRefresh} setShowAiDetailView={setShowAiDetailView} itemData={itemData} vendorId={vendorId} /> : <DataTable
        value={menuItem}
        size="small"
        resizableColumns
        paginator
        ...
        emptyMessage={tableLoader ? <div className="flex flex-wrap mt-4 items-center justify-center ">
          ...
          <Loader />
        </div> : <p className="text-main-color">Products Not Found</p>}
        ...
        className="  w-full  p-2  mt-2 overflow-x-hidden all-customers text-main-color font-poppins "
      >
        ...
        <Column header="Stock/Out" headerClassName="font-normal text-white bg-main-color" className="text-main-color font-normal" field="totalStocks"
          body={(rowData) => {
            return (
              <p>{rowData?.totalStocks === 0 ? "Out Of Stock" : "In Stock"}</p>
            )
          }}
        ></Column>
