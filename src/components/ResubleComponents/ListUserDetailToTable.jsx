...
export default function ListUserDetailToTable({ customer, index }) {
  const [checked, setChecked] = useState(false);
  return (
    <tr key={index} className="text-main-color">
      <td className="px-6  whitespace-no-wrap">
        {customer.image && customer.nameDriver ? (
    ...
    e.target.src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTw_HeSzHfBorKS4muw4IIeVvvRgnhyO8Gn8w&s"
          ...
          customer.customerName || customer.customerid || customer.nameDriver
        )}
      </td>

      <td className="px-1 py-4 whitespace-no-wrap  ">
        <div className="flex gap-3  ">
          {customer.image && !customer.tips ? (
            <img onError={(e)=>{ 
    e.target.src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTw_HeSzHfBorKS4muw4IIeVvvRgnhyO8Gn8w&s"
  }}src={customer.image} className="w-9 h-9" />
          ) : (
    ...
    e.target.src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTw_HeSzHfBorKS4muw4IIeVvvRgnhyO8Gn8w&s"
      ...
      <td className="px-6 py-4 whitespace-no-wrap">
        {customer.amount || customer.city || customer.profit || customer.date}
      </td>

      {customer.vat && <td className=" whitespace-no-wrap">{customer.vat}</td>}
      ...
      {!customer.city && !customer.vat && !customer.ref && !customer.tips ? (
      ...
      <td className="px-6 py-4 whitespace-no-wrap  whitespace-no-wrap flex items-center   justify-center">
        {" "}
        <Button
          icon="pi pi-pencil"
