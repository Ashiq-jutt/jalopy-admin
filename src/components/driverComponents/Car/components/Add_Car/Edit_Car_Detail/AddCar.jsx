 ...
 const imagRef=useRef()
  ...
  const validationSchema=Yup.object().shape({
    ...
    RegisterationDate: Yup.string().required("Registration Date Is Required"),     
     Color:Yup.string().required("Color Is Required"),   
      ...
      Tuv:Yup.string().required("Tuv File  Is Required"),  
       TuvDate:Yup.string().required("Tuv Is Required"),
      License:Yup.string().required("License File  Is Required"),    
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, {resetForm}) => {        
      ...
      Axios.post(`${process.env.REACT_APP_BASE_URL}/api/v1/Vehicles/Create`,formData,{headers:{'Authorization':`Bearer ${token}`}}).then((res)=>{ 
        toast.current.show({ severity: "success", summary: "Info", detail: <p className="font-poppins">{res?.data?.message ? res?.data?.message :"Car Added Sucessfully"}</p> });
        resetForm()           
        setLoaderShow(false) 
      
      LicenseRef.current.value = ''; 
      tuvRef.current.value=''
           registerationRef.current.value=''

      }).catch((error)=>{           
        
        toast.current.show({ severity: "error", summary: "Info", detail: <p className="font-poppins">{error?.response?.data?.Message ? error?.response?.data?.Message :"Car Addition Failed"}</p> });
        setLoaderShow(false)
      
    })}
  }); 
  return (
    <form onSubmit={formik.handleSubmit} className="add-car-comp">  
