...
export default function AllConnections({ setPartnerHide, allConnectionHide, setAllConnectionHide, accountactive, toast, token, setConversationUser }) {
  ...
  return (
    <>
      {
        allConnectionHide ? <i onClick={() => {
          setAllConnectionHide(false)
        }} className="pi left-[8px] absolute cursor-pointer text-main-color pi-bars md:hidden" /> :
          ""
      }
      <div className={`${allConnectionHide ? "hidden" : "w-[98%]"} md:w-[33%] md:block absolute   md:relative z-10 ${accountactive ? "h-[calc(100vh-100px)] md:h-[calc(100vh-270px)]" : "h-[calc(100vh-175px)]"} overflow-y-auto   bg-[white] `}>
        <i onClick={() => {
          setPartnerHide(false)
          setAllConnectionHide(true)
        }} className={` ${!allConnectionHide ? "" : ""}  cursor-pointer md:hidden text-main-color pi pi-arrow-left`} />
